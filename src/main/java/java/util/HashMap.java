...
public class HashMap<K,V> extends AbstractMap<K,V> implements Map<K,V>, Cloneable, Serializable {
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
         ...
         *          c.2. 插入后, 一共有6种情况:
         *              c.2.1. 左三(中左左*) 黑红红, 不符合红黑树定义 => 需要调整, 左1 右旋, 左1 左2 变色
         *              c.2.2. 中左右*(其实就相当于左三, 因为对父结点进行左旋即得到左三) 黑红红, 不符合红黑树定义 => 需要调整, 父结点左旋, 左1 右旋, 左1 左2 变色
         *              c.2.3. 右三(中右右*) 黑红红, 不符合红黑树定义 => 需要调整, 右1 左旋, 右1 右2 变色
         *              c.2.4. 中右左*(其实就相当于右三, 因为对父结点进行右旋即得到右三) 黑红红, 不符合红黑树定义 => 需要调整, 父结点右旋, 右1 左旋, 右1 右2 变色
         *              c.2.5. 中左右* 黑红红, 符合红黑树定义 => 无需调整
         *              c.2.6. 中左*右 黑红红, 符合红黑树定义 => 无需调整
         *      d. 合并到4结点 => 成为一个裂变状态
         *         d.1. 插入前是一个4结点(中左右 黑红红)
         *         d.2. 插入后, 一共有4种情况:(由于是红黑树, 本质就是二叉树, 所以4结点合并无需考虑向上升元的情况)
         ...
         * 调整的3种情况:
         *      a. 自己能搞定的自己搞定:
         *          a.1. 如果删除的结点对应于2-3-4树的3结点或4结点(相当于情况C.2、C.1、B、红结点的情况A) => 则直接删除, 不用跟兄弟和父亲借, 无需调整
         *          a.2. 删除3结点时, 如果删除的是红结点 => 则直接删除, 无需调整
         *          a.3. 删除3结点时, 如果删除的是黑结点 => 则删除后用红结点替代, 然后变黑即可
         *      b. 搞不定的找兄弟和父亲帮忙(即删除的是黑色的2结点, 相当于黑结点的情况A):
         *          => 前提是找到真正得兄弟结点:
         *              b.0.1. 如果兄弟结点为红色, 说明不是真正的兄弟结点(可能是为2结点的父结点的红结点), 因为兄弟结点无论是2、3、4结点, 都是黑结点在上的
         *              b.0.2. 这时, 需要对父结点进行左旋, 把本来属于父结点的红结点作为新的父结点, 这样出现在兄弟结点位置的结点才是真正的兄弟结点(黑色)
         *          b.1. 兄弟有得借(即兄弟结点为3结点或4结点):
         *              b.1.1. 兄弟结点为3结点, 即只有一个子结点时 => 需要对父结点旋转, 使得该兄弟结点作为新得父结点
         *              b.1.2. 兄弟结点为4结点, 即有两个子结点时(子结点肯定是红色) => 这里调整方案有2个:
         *                  b.1.2.1. 兄弟结点借出去1个 => 需要先对兄弟结点旋转, 转出最左的子结点, 然后对父结点旋转, 把该子结点转到父结点位置, 即需要进行两次旋转; 变色;
         *                  b.1.2.2. 兄弟结点借出去2个 => 需要对父结点行旋转, 把兄弟结点转到父结点位置, 兄弟结点的左子结点转到父结点的右孩子处, 父结点转到兄弟结点的左孩子处, 即只需1次旋转; 变色;
         *                      => 同时还需要注意, 如果当前结点为父结点的左孩子, 兄弟结点没有右孩子, 则还需要先旋转出左孩子, 防止空的右孩子破坏旋转后的兄弟结点成为父结点
         *                      => 同时还需要注意, 如果当前结点为父结点的右孩子, 兄弟结点没有左孩子, 则还需要先旋转出右孩子, 防止空的左孩子破坏旋转后的兄弟结点成为父结点
         *          b.2. 兄弟没得借(即兄弟结点为2结点, 对应红黑树一定是黑结点):
         *              b.2.1. 删除该黑色的2结点后, 此时该节点原所在分支比兄弟结点所在分支的黑色结点数量少1 => 需要对兄弟结点所在分支的黑色结点自损, 有难同当:
         *                  => 自损的方式最快的是:
         *                          b.2.1.1. 将兄弟结点变红, 此时父结点所在的子树黑色平衡
         *                          b.2.1.2. 但父结点以上等祖结点的子树不平衡, 此时需要进行向上递归处理
         *                          b.2.1.3. 直到新父结点为根结点(向上没有别的子树) 或者 新父结点为红结点(因为这时右子树已经变色, 所以只需要将当前新父结点变为黑色即可)
         */
        static <K,V> TreeNode<K,V> balanceDeletion(TreeNode<K,V> root, TreeNode<K,V> x) {
            ...
            for (TreeNode<K,V> xp, xpl, xpr;;)  {
                ...
                // 20201122 当前结点x为黑结点(删除2结点 或者 3结点的黑结点时), 如果当前结点x属于父节点的左孩子, 则按左的方式调整 => 这里的xpl也等于x为右孩子时的叔结点
                    ...
                    // 20201122 如果叔结点xpr为父节点的右孩子, 且为红结点时(不是真正的兄弟结点, 只是为2结点父结点的红结点)
                    ...
                    // 20201122 如果没有真正的兄弟结点, 说明兄弟没得借(对应b.2. 兄弟没得借)
                    if (xpr == null)
                        x = xp;// 20201122 则设置父亲结点为新的当前结点, 开始向上自损, 向上递归
                    ...
                    // 20201122 如果真正的兄弟结点存在(肯定为黑结点)
                        ...
                        // 20201122 兄弟结点任意一个子结点为红色, 说明兄弟结点有得借(对应方案b.1.2.2. 兄弟结点借出去2个)
                            ...
                            // 20201122 如果兄弟结点的右孩子没得借, 那只能旋出左孩子了 => 防止右结点为空, 只旋转一次破坏2结点的情况
                            if (sr == null || !sr.red) {
                ...
                // 20201122 同理, 如果当前结点x属于父节点的右孩子时, 则进行反向操作
                    ...
                    // 20201122 xpl在上面已经获取到了, 兄弟结点为红结点, 说明该兄弟结点不是真正的兄弟结点(因为这时为2结点的父结点的红结点)
                    if (xpl != null && xpl.red) {
                        // 20201122 设置假兄弟结点(为2结点的父结点的红结点)为黑色 => 为了保持旋转后父结点2结点的性质
                        xpl.red = false;

                        // 20201122 设置父结点(为2结点的父结点的黑结点)为红色 => 为了保持旋转后父结点2结点的性质
                        xp.red = true;

                        // 20201122 对父结点进行右旋, 把为2结点的父结点的红结点旋上父结点位置, 旋出真正的兄弟结点
                    ...
                    // 20201122 如果真正的兄弟结点为空, 说明兄弟结点没得借(对应b.2. 兄弟没得借)
                    if (xpl == null)
                        x = xp;// 20201122 则设置父亲结点为新的当前结点, 开始向上自损, 向上递归
                    ...
                    // 20201122 如果真正的兄弟结点存在(肯定为黑结点)
                        ...
                        // 20201122 兄弟结点任意一个子结点为红色, 说明兄弟结点有得借(对应方案b.1.2.2. 兄弟结点借出去2个)
                            ...
                            // 20201122 如果兄弟结点的左孩子没得借, 那只能旋出右孩子了 => 防止左结点为空, 只旋转一次破坏2结点的情况
                            if (sl == null || !sl.red) {
                                if (sr != null)
